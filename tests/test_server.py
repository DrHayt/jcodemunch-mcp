...
async def test_server_lists_all_tools():
    ...
    try:
        ...
        assert len(tools) == 70  # +1: import_runtime_signal (Phase 1, v1.97.0)
...
async def test_disabled_tools_filtered_from_schema(monkeypatch):
    ...
    try:
        ...
        # 70 default tools + test_summarizer (config cleared) - 2 disabled = 69
...
async def test_disabled_tools_empty_all_tools_present(monkeypatch):
    """When disabled_tools is empty, all 71 tools are present (70 + test_summarizer)."""
    from jcodemunch_mcp import config as config_module
    ...
    try:
        ...
        tools = await list_tools()
        assert len(tools) == 71  # 70 + test_summarizer (config cleared, so disabled gate off)
    finally:
        config_module._GLOBAL_CONFIG.clear()
        config_module._GLOBAL_CONFIG.update(orig_config)
