...
from ..runtime import VALID_SOURCES, ingest_otel_file
    ...
    *,
    ...
    Args:
        source: One of ``{'otel', 'sql_log', 'stack_log', 'apm'}``. Phase 1
            implements ``'otel'``; the others land in Phase 4+.
    ...
    if source != "otel":
        return {
            "success": False,
            "error": (
                f"source {source!r} is not yet implemented (Phase 4+). "
                "Phase 1 supports source='otel'."
            ),
        }
